...
def main():
    ...
    for polish_range in polish_ranges:
        filename_polish_range = polish_range.replace(':','_')
        # job_name = job_prefix + polish_range
        variants_command = nanopolish_exec + ' variants --consensus -o polished.' + filename_polish_range + '.fa -w ' + polish_range + ' -r ' + read_filename + ' -b reads.sorted.bam -g ' + assembly_filename + ' -t 12 --min-candidate-frequency 0.1'
        if methylation_aware:
        ...
        sbatch_command = variants_command
    ...
    incomplete_ranges = [x.replace(':','_') for x in polish_ranges if not os.path.isfile('polished.' + x.replace(':','_') + '.fa')]
    ...
    for incomplete_range in incomplete_ranges:
        ...
        start = int(incomplete_range.split('_')[-1].split('-')[0])
