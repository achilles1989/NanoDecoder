...
python_script_dir=/media/quanc/D/Workspace/python/NanoDecoder/scripts

# Set the full path to Nanopolish here.
nanopolish_exec_dir=/media/quanc/D/Workspace/github/nanopolish
...
#read_files="albacore-2.3.3.fasta"
#read_files="NanoDecoder-seq-300-stride-300-beam-1-max-50.fasta"
#read_files="scrappie-1.4.0.fasta"
...
for f in $read_files; do
    ...
    assembly_delta=08_assembly_data/"$set"_assembly
    assembly_snps=08_assembly_data/"$set"_assembly.snps
    assembly_snps_anno=08_assembly_data/"$set"_assembly_errors.tsv
...
#    minimap2 -k12 -t $threads -c reference.fasta $all_reads_fixed_names > $read_alignment
    minimap2 -x map-ont -t $threads -c reference.fasta $all_reads_fixed_names > $read_alignment
...
#    filtlong --min_length 1000 --target_bases 500000000 $trimmed_reads | gzip > $subsampled_reads
...
#    minimap2 -k12 -t $threads -c reference.fasta $assembly_pieces > $assembly_alignment
    minimap2 -x map-ont -t $threads -c reference.fasta $assembly_pieces > $assembly_alignment
    python3 "$python_script_dir"/read_length_identity.py $assembly_pieces $assembly_alignment > $assembly_data
    nucmer -t $threads --maxmatch -p $assembly_delta reference.fasta $assembly
    show-snps -H -T -l -q -x 4 $assembly_delta.delta > $assembly_snps
    python3 "$python_script_dir"/assembly_errors.py $assembly_snps > $assembly_snps_anno
    rm $assembly_pieces $assembly_alignment $assembly_snps $assembly_delta.delta
